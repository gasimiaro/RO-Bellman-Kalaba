...
reversed_arbre = getReversedTree(arbre,first_node)

################################################################################

def getAllMinPotential(tree):
    potential = {k: float('inf') for k in arbre.keys()}  
    potential[last_node] = 0
        ...
        #    if step in s:               
                for child_node,val in child.items():
                    ...
                    if potential[child_node] != "inf" and new_pot < potential[parent_node]:
                        ...
                        changed = True
        step += 1
    return {'step' : step -1,'potential' :potential}
################################################################################
...
#   new_tree = tree.copy()
  ...
  for node, (children,_) in new_tree.items():
      for child_node,weight in children.items():
          if node in new_tree[child_node][0].keys() :
...
def addPathsTree(tree, AllPaths):
...
def getAllMaxPotential(tree):
    potential = {k: float('-inf') for k in max_arbre.keys()}  
    potential[last_node] = 0
    step = 1
    changed = True
